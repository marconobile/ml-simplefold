#!/usr/bin/env python3
"""Align a CIF structure to an NPZ trajectory frame and compute backbone RMSD.

The script performs rigid-body alignment (Kabsch, no scaling) and reports:
- global post-alignment RMSD on backbone atoms only
- per-atom backbone distances after alignment (optionally written to CSV)

It includes strict atom-pairing checks and will fail if it cannot establish a
1:1 mapping between CIF and NPZ backbone atoms using reliable keys.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "backbone-only RMSD with strict atom-pairing checks."
        )
    )
    parser.add_argument(
        "--cif-path",
...
def _backbone_mask(atom_names: np.ndarray) -> np.ndarray:
    return np.isin(atom_names, BACKBONE_ATOM_NAMES)


def filter_cif_backbone(cif_atoms: CifAtoms) -> CifAtoms:
    mask = _backbone_mask(cif_atoms.atom_names)
    if not np.any(mask):
        raise ValueError("No backbone atoms found in selected CIF atoms.")
    return CifAtoms(
        coords=cif_atoms.coords[mask],
        atom_names=cif_atoms.atom_names[mask],
        residue_ids=cif_atoms.residue_ids[mask],
        residue_indices=cif_atoms.residue_indices[mask],
        residue_names=cif_atoms.residue_names[mask],
        chain_ids=cif_atoms.chain_ids[mask],
        insertion_codes=cif_atoms.insertion_codes[mask],
    )


def filter_npz_backbone(npz_atoms: NpzAtoms) -> tuple[NpzAtoms, np.ndarray]:
    mask = _backbone_mask(npz_atoms.atom_names)
    if not np.any(mask):
        raise ValueError("No backbone atoms found in NPZ atom metadata.")
    source_indices = np.flatnonzero(mask)
    return (
        NpzAtoms(
            coords=npz_atoms.coords[mask],
            atom_names=npz_atoms.atom_names[mask],
            atom_resids=npz_atoms.atom_resids[mask]
            if npz_atoms.atom_resids is not None
            else None,
            atom_residue_index=npz_atoms.atom_residue_index[mask]
            if npz_atoms.atom_residue_index is not None
            else None,
        ),
        source_indices,
...
def main() -> None:
    ...
    npz_atoms_all, npz_info = load_npz_atoms(
        npz_path=args.npz_path.resolve(),
        frame_index=args.frame_index,
    )

    cif_atoms = filter_cif_backbone(cif_atoms_all)
    npz_atoms, npz_backbone_source_indices = filter_npz_backbone(npz_atoms_all)
    ...
    npz_indices_for_cif_order = npz_backbone_source_indices[mapping.npz_indices_for_cif_order]

    reference_xyz = cif_atoms.coords
    mobile_xyz = npz_atoms_all.coords[npz_indices_for_cif_order]
    ...
    print(f"Backbone atoms      : {', '.join(BACKBONE_ATOM_NAMES)}")
    print(f"CIF atoms (all)     : {cif_atoms_all.coords.shape[0]}")
    print(f"CIF atoms (backbone): {reference_xyz.shape[0]}")
    print(f"NPZ atoms (all)     : {npz_atoms_all.coords.shape[0]}")
    print(f"NPZ atoms (backbone): {npz_atoms.coords.shape[0]}")
    ...
    if args.per_atom_out is not None:
        ...
        write_per_atom_csv(
            out_path=out_path,
            cif_atoms=cif_atoms,
            npz_indices=npz_indices_for_cif_order,
            distances=distances,
        )
